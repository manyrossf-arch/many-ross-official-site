      ...
      <div className="grid gap-6 xl:grid-cols-3">
        {releases.map((release, index) => (
          <FadeIn key={release.title} delay={0.12 + index * 0.08}>
            <article className="glass-panel group min-w-0 overflow-hidden rounded-[34px]">
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute left-4 top-4 max-w-[calc(100%-2rem)] rounded-full border border-white/10 bg-black/35 px-3 py-2 text-[10px] uppercase tracking-[0.16em] text-gold compact-eyebrow">
              ...
              <div className="space-y-4 p-6 min-w-0">
                ...
                <ExternalLink
                  href={release.href}
                  className="inline-flex min-h-[3rem] max-w-full items-center rounded-full border border-gold/30 bg-gold/10 px-4 py-2 text-center text-xs uppercase tracking-[0.18em] text-gold transition hover:border-gold/60 hover:bg-gold/15 whitespace-normal compact-tracking"
                >
