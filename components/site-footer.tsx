      ...
      <div className="section-shell space-y-8 py-12">
        <div className="grid gap-8 md:grid-cols-[1.4fr_1fr_1fr]">
          <div className="min-w-0 space-y-4">
            ...
            <p className="mobile-safe-copy max-w-xl text-sm leading-7 text-white/62">
              Musica, videos, comunidad, plataformas oficiales y lanzamientos disenados como una sola experiencia premium.
            </p>
          </div>

          <div className="min-w-0 space-y-3">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-gold compact-tracking">
              Plataformas
            </p>
            <div className="flex flex-col gap-2">
              {platformLinks.map((item) => (
                <ExternalLink key={item.label} href={item.href} className="text-sm text-white/65 transition hover:text-ivory text-anywhere">
                  {item.label}
                </ExternalLink>
              ))}
            </div>
        ...
        <div className="flex flex-col gap-3 text-sm text-white/45 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-balance">&copy; 2026 Many Ross. Todos los derechos reservados.</p>
          <p className="text-balance">Disenado para escalar como marca internacional.</p>
        </div>
      </div>
    </footer>
  );
